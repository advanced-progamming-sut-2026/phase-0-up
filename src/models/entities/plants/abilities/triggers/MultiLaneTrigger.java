...
public class MultiLaneTrigger implements TriggerStrategy {
    ...
    public MultiLaneTrigger(int[] relativeRows, boolean targetsObstacles) {
    ...
    public boolean canTrigger(Plant owner, GameSession gameSession) {
            ...
            if (targetRow >= 0 && targetRow < Constants.BOARD_ROWS) {
                ...
                if (zombiesInRow != null) {
                    for (Zombie z : zombiesInRow) {
                        if (Targets.reachable(owner, z) && z.getMovement().getPositionX() > ownerX) {
                            return true;
                        }
                    }
                }
        ...
        return targetsObstacles && ObstacleSight.obstacleInLanes(owner, gameSession, relativeRows);
    }
}