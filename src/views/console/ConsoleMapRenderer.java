...
// Draws the board as ASCII. The terminal build's MapRenderer.
public class ConsoleMapRenderer implements MapRenderer {
    ...
    // Private, not part of the MapRenderer contract: these two listings are how the TERMINAL chooses to
    // spell out a board it can only draw in one character per cell. A graphical build shows the same
    // information by drawing the entities, so there is nothing for it to implement.
    private void renderAllZombies(List<Zombie> activeZombies){
    ...
    private void renderAllPlants(List<Cell> cells){
        if (cells.isEmpty()) {
