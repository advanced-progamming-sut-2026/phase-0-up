...
public final class NetClient implements AutoCloseable {
    ...
    // A SynchronousQueue per in-flight request: the reader thread hands the reply straight to the
    // waiting caller. A queue rather than a shared lock so two requests in flight at once (the game
    // never does this today, but a background sync alongside a menu action would) cannot collect each
    // other's answers.
    private final Map<Long, SynchronousQueue<Envelope>> pending = new ConcurrentHashMap<>();
    ...
    public <T extends Packet> T request(Packet request, Class<T> expected) {
        ...
        SynchronousQueue<Envelope> slot = new SynchronousQueue<>();
        ...
        try {
            ...
            if (!expected.isInstance(reply.payload())) {
                // Usually the auth gate answering with an AckResponse where the caller expected
                // something richer. Naming both sides makes that immediately readable in a log.
    ...
    private void onPacket(Envelope envelope) {
        if (envelope.isReply()) {
            SynchronousQueue<Envelope> slot = pending.get(envelope.correlationId());
            // offer, not put: if the caller has already timed out and gone, there is nobody to hand it
            // to, and blocking here would wedge the reader thread for the whole session.
    ...
    private void onDisconnected() {
        ...
        for (SynchronousQueue<Envelope> slot : pending.values()) {
